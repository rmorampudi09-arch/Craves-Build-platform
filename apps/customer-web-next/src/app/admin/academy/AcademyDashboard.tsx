...
import { ArrowRight, BookOpen, Check, ChevronLeft, ChevronRight, Code2, Flame, GraduationCap, Layers3, Lightbulb, LockKeyhole, Play, Pause, Search, ShieldCheck, Sparkles, Target, Trophy, Volume2, X } from "lucide-react";
...
async function api<T>(path: string, method = "GET", body?: unknown): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) throw new Error(response.status === 401 ? "Sign in again using your Craves administrator account." : response.status === 403 ? "Your administrator role does not permit this action." : data?.message || data?.detail || `Academy request unavailable (${response.status}). Check that the service and API route are enabled.`);
    return data as T;
...
export default function AcademyDashboard() {
  ...
  const [error, setError] = useState(""); const [view, setView] = useState("library");
  ...
  const refresh = useCallback(async () => { setMe(await api<Learner>("me")); }, []);
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCatalog(nextCatalog); setMe(nextMe);
  ...
  function changeView(next: string) { setView(next); if (next !== "learn") window.history.replaceState(null, "", "/admin/academy"); }
  if (!catalog || !me) return <div className="ca-root ca-empty"><GraduationCap size={42} /><h1>Craves Academy</h1><p role={error ? "alert" : "status"}>{error || "Loading your protected learning workspace…"}</p>{error && <button className="ca-primary" onClick={() => void load()}>Retry connection</button>}<small>No sample learner records or simulated production status are shown.</small></div>;
  ...
  return <div className="ca-root">
    <div className="ca-topline"><span><GraduationCap size={18} /> CRAVES / ACADEMY</span><span className="ca-pill"><LockKeyhole size={12} /> Internal learning</span></div>
    <div className="ca-title"><div><h1>Know Craves. Build better.</h1><p>Your product. Your services. The source behind every decision.</p></div><span className="ca-version">SOURCE <strong>{catalog.sourceRevision.slice(0, 8)}</strong></span></div>
    ...
    {error && <div className="ca-alert" role="alert">{error}<button onClick={() => setError("")} aria-label="Dismiss message"><X size={16} /></button></div>}
    ...
    {view === "library" && <>
      ...
      <section className="ca-library"><div className="ca-section-title"><div><span className="ca-kicker">BUILT AROUND YOUR PRODUCT</span><h2>Explore the service library <small>{catalog.courses.length}</small></h2></div><label className="ca-sort">Sort by<select value={sort} onChange={e => setSort(e.target.value)}><option value="path">Learning path</option><option value="az">A–Z</option><option value="short">Shortest first</option><option value="progress">Progress</option></select></label></div>
        ...
        <div className="ca-course-grid">{filtered.map(c => <article className={`ca-course ca-tone-${catalog.courses.indexOf(c) % 4}`} key={c.id}>
        ...
        {!filtered.length && <div className="ca-empty"><Search /><h3>No matching courses</h3><p>Try another service or a broader search term.</p><button className="ca-primary" onClick={() => { setSearch(""); setService("all"); }}>Clear filters</button></div>}
      </section>
...
function LessonRoom({ course, lesson, catalog, onProgress }: { course: Course; lesson: Lesson; catalog: Catalog; onProgress: () => Promise<void> }) {
  ...
  const [result, setResult] = useState<QuizResult | null>(null), [error, setError] = useState(""), [busy, setBusy] = useState(false);
  ...
  useEffect(() => {
    ...
    const record = (kind: string) => activity(course.id, lesson.id, kind).catch(() => { if (alive) setError("Learning activity could not be saved. Quiz submissions have their own confirmed save result."); });
  ...
  async function loadSource(index: number) {
    const sequence = ++sourceSequence.current; setSourceBusy(true); setSource(null); setError(""); setSourceIndex(index);
    ...
    catch (e) { if (sequence === sourceSequence.current) setError(message(e)); }
  ...
  async function submit() {
    ...
    try {
      ...
      setResult(result); await onProgress();
    } catch (e) { setError(message(e)); } finally { setBusy(false); }
  }
  return <section className="ca-lesson-main"><div className="ca-lesson-head"><span className="ca-kicker">SECTION {course.lessons.indexOf(lesson) + 1} / {course.lessons.length}</span><h2>{lesson.title}</h2><div className="ca-tabs"><button aria-current={tab === "lesson" ? "page" : undefined} onClick={() => setTab("lesson")}><BookOpen size={15} /> Walkthrough</button><button aria-current={tab === "guided" ? "page" : undefined} onClick={() => setTab("guided")}><Play size={15} /> Guided lesson</button><button aria-current={tab === "source" ? "page" : undefined} onClick={() => { setTab("source"); if (!source && !sourceBusy) void loadSource(sourceIndex); }}><Code2 size={15} /> Source code</button><button aria-current={tab === "quiz" ? "page" : undefined} onClick={() => setTab("quiz")}><Target size={15} /> Section quiz</button></div></div>
    {error && <p className="ca-alert" role="alert">{error}</p>}
...
function Roadmap({ catalog, canManage }: { catalog: Catalog; canManage: boolean }) {
  const [plans, setPlans] = useState<Plan[]>([]), [error, setError] = useState(""), [loading, setLoading] = useState(true), [busy, setBusy] = useState(false);
  ...
  const reload = useCallback(async () => { setLoading(true); try { setPlans(await api<Plan[]>("plans")); } catch (e) { setError(message(e)); } finally { setLoading(false); } }, []);
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api("plans", "PUT", { id: edit?.id || crypto.randomUUID(), title, service, details, status: stage, expectedRevision: edit?.revision || 0 }); setEdit(null); setTitle(""); setDetails(""); setStage("PROPOSED"); await reload(); }
    catch (e) { setError(message(e)); } finally { setBusy(false); }
  }
  return <section className="ca-roadmap"><div className="ca-section-title"><div><span className="ca-kicker">KEEP IDEAS DISTINCT FROM FACTS</span><h2>Future plans, with context.</h2><p>Private runtime records. No unpublished business plans are seeded into the public repository.</p></div><select value={status} aria-label="Filter plans by status" onChange={e => setStatus(e.target.value)}><option value="all">All stages</option>{["PROPOSED", "APPROVED", "IN_PROGRESS", "SHIPPED"].map(s => <option key={s}>{s}</option>)}</select></div>{error && <p className="ca-alert" role="alert">{error}</p>}{loading ? <p role="status">Loading plans…</p> : <div className="ca-plan-grid">{plans.filter(p => status === "all" || p.status === status).map(p => <article className="ca-panel" key={p.id}><span className="ca-pill">{p.status.replaceAll("_", " ")}</span><h3>{p.title}</h3><small>{p.service} · revision {p.revision}</small><p className="ca-plan-detail">{p.details}</p>{canManage && <div className="ca-row"><button onClick={() => choose(p)}>Edit plan</button><button className="ca-danger" onClick={async () => { if (!window.confirm(`Delete the private plan “${p.title}”?`)) return; try { await api(`plans/${p.id}?revision=${p.revision}`, "DELETE"); await reload(); } catch (e) { setError(message(e)); } }}>Delete</button></div>}</article>)}</div>}{!loading && !plans.length && <div className="ca-empty"><Lightbulb size={34} /><h3>No future plans published yet</h3><p>A platform administrator can add approved internal context below. Proposed work is never presented as implemented behavior.</p></div>}{canManage && <form className="ca-panel ca-plan-form" onSubmit={save}><h3>{edit ? "Edit private plan" : "Publish an internal plan"}</h3><label>Plan title<input required maxLength={160} value={title} onChange={e => setTitle(e.target.value)} /></label><div className="ca-form-grid"><label>Service area<select value={service} onChange={e => setService(e.target.value)}><option value="platform">Platform</option>{catalog.courses.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}</select></label><label>Evidence stage<select value={stage} onChange={e => setStage(e.target.value)}>{["PROPOSED", "APPROVED", "IN_PROGRESS", "SHIPPED"].map(s => <option key={s}>{s}</option>)}</select></label></div><label>Context, rationale and evidence<textarea required maxLength={8000} rows={6} value={details} onChange={e => setDetails(e.target.value)} placeholder="Explain the change, owner, dependencies and approval or deployment evidence. Do not enter credentials or customer data." /></label><div className="ca-row"><button className="ca-primary" disabled={busy}>{busy ? "Saving…" : "Save private plan"}</button>{edit && <button type="button" onClick={() => { setEdit(null); setTitle(""); setDetails(""); }}>Cancel editing</button>}</div></form>}</section>;
}

function Insights() {
  const [report, setReport] = useState<Report | null>(null), [page, setPage] = useState(0), [error, setError] = useState("");
  useEffect(() => { let active = true; setReport(null); api<Report>(`analytics?page=${page}`).then(r => { if (active) { setReport(r); setError(""); } }).catch(e => { if (active) setError(message(e)); }); return () => { active = false; }; }, [page]);
  if (error) return <p role="alert" className="ca-alert">{error}</p>;
  if (!report) return <p role="status">Loading authorized team insights…</p>;
