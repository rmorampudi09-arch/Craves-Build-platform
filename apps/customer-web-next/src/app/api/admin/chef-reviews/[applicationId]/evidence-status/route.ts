...
export async function GET(
  ...
  try {
    ...
    if (!Array.isArray(body) || body.length > 20) return NextResponse.json({ code: "INVALID_EVIDENCE_STATUS_RESPONSE" }, { status: 502 });
    const items = body.map(sanitize);
    if (items.some(item => item === null)) return NextResponse.json({ code: "INVALID_EVIDENCE_STATUS_RESPONSE" }, { status: 502 });
