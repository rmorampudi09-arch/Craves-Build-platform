...
export async function POST(request:NextRequest,{params}:{params:Promise<{dataset:string}>}) {
  ...
  try {
    ...
    if(!upstream.ok){ await upstream.body?.cancel();return fail(upstream.status===403?"EXPLORER_ACCESS_REQUIRED":upstream.status===401?"AUTHENTICATION_REQUIRED":upstream.status===400?"INVALID_EXPLORER_QUERY":upstream.status===429?"EXPLORER_BUSY":"EXPLORER_UNAVAILABLE",[400,401,403,429].includes(upstream.status)?upstream.status:503); }
