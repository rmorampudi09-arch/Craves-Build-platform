...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!session) return NextResponse.json({ code: "INVALID_AUTH_RESPONSE", message: "Sign-in is temporarily unavailable." }, { status: 502 });
    const response = NextResponse.json({ identity: session.identity }, { headers: { "Cache-Control": "no-store" } });
    setSessionCookies(response, session);
    return response;
  } catch (error) {
    const timedOut = error instanceof Error && error.name === "AbortError";
    return NextResponse.json({ code: timedOut ? "AUTH_TIMEOUT" : "AUTH_UNAVAILABLE", message: timedOut ? "Sign-in timed out. Please try again." : "Sign-in is temporarily unavailable." }, { status: timedOut ? 504 : 503 });
  } finally {
