...
import {
  parseChefProofDocument,
  type ChefDocumentType,
...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    !ALLOWED_TYPES.has(documentType as ChefDocumentType) ||
    !(file instanceof File)
  ) {
    return NextResponse.json(
      { code: "INVALID_PROOF_FILE_REQUEST" },
      { status: 400 },
    );
  }
  if (
  ...
  try {
      ...
      const response = NextResponse.json(
        {
          ...
          message:
            upstream.status === 401
              ...
              : upstream.status === 400
                ? "The proof file or document type was rejected. Use a PDF, JPG or PNG file under 10 MB."
                : upstream.status === 409
    ...
    const document = parseChefProofDocument(
      await upstream.json().catch(() => null),
    );
    if (!document)
      return NextResponse.json(
        { code: "INVALID_PROOF_FILE_RESPONSE" },
        { status: 502 },
      );
