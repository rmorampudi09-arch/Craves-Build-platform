...
async function call(request: NextRequest, method: "GET" | "PUT", body?: unknown) {
  ...
  try {
    ...
    if (method === "GET" && upstream.status === 404) return NextResponse.json(null, { status: 200, headers: { "Cache-Control": "no-store" } });
    if (!upstream.ok) {
      const response = NextResponse.json({ code: upstream.status === 401 ? "SESSION_EXPIRED" : upstream.status === 403 ? "CHEF_ACCESS_REQUIRED" : "KITCHEN_REQUEST_FAILED" }, { status: upstream.status });
    ...
    if (!kitchen) return NextResponse.json({ code: "INVALID_KITCHEN_RESPONSE" }, { status: 502 });
    const response = NextResponse.json(kitchen);
    response.headers.set("Cache-Control", "no-store");
    return response;
  } catch (error) {
    const timedOut = error instanceof Error && error.name === "AbortError";
    return NextResponse.json({ code: timedOut ? "KITCHEN_TIMEOUT" : "KITCHEN_UNAVAILABLE" }, { status: timedOut ? 504 : 503 });
