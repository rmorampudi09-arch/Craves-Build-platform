...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const identity = parseChefModeIdentity(await upstream.json().catch(() => null));
    ...
    return NextResponse.json({ code: timedOut ? "IDENTITY_TIMEOUT" : "IDENTITY_UNAVAILABLE" }, { status: timedOut ? 504 : 503 });
  } finally {
