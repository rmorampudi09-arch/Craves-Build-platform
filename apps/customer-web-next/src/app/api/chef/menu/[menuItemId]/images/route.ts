...
export async function POST(request: NextRequest, context: { params: Promise<{ menuItemId: string }> }) {
  ...
  try {
    const upstream = await boundedFetch(`${apiBaseUrl()}/kitchens/me/menu-items/${encodeURIComponent(menuItemId)}/images?primary=${primary}`, { method: "POST", headers: { Authorization: `Bearer ${token}`, Accept: "application/json" }, body: upstreamForm, cache: "no-store", signal: controller.signal }, 40_000);
