...
export async function POST(request: NextRequest, context: { params: Promise<{ orderId: string }> }) {
  ...
  const raw = await request.json().catch(() => null) as { prepTimeMinutes?: unknown; note?: unknown; actionId?: unknown } | null;
  const prepTimeMinutes = typeof raw?.prepTimeMinutes === "number" && Number.isInteger(raw.prepTimeMinutes) ? raw.prepTimeMinutes : 0;
  ...
  if (prepTimeMinutes < 1 || prepTimeMinutes > 1440 || !UUID.test(actionId)) return NextResponse.json({ code: "INVALID_ACCEPTANCE_REQUEST" }, { status: 400 });
  ...
  try {
    const upstream = await fetch(`${apiBaseUrl()}/chef/orders/${encodeURIComponent(orderId)}/accept`, { method: "POST", headers: { Authorization: `Bearer ${token}`, Accept: "application/json", "Content-Type": "application/json", "X-Correlation-ID": actionId, "Idempotency-Key": actionId }, body: JSON.stringify({ prepTimeMinutes, note }), cache: "no-store", signal: controller.signal });
    if (!upstream.ok) { const response = NextResponse.json({ code: upstream.status === 401 ? "SESSION_EXPIRED" : upstream.status === 409 ? "CHEF_ACCEPTANCE_CONFLICT" : "CHEF_ACCEPTANCE_FAILED" }, { status: upstream.status }); if (upstream.status === 401) response.cookies.delete("craves_access_token"); return response; }
  ...
  } catch (error) { const timedOut = error instanceof Error && error.name === "AbortError"; return NextResponse.json({ code: timedOut ? "CHEF_ACCEPTANCE_TIMEOUT" : "CHEF_ACCEPTANCE_UNAVAILABLE" }, { status: timedOut ? 504 : 503 }); }
