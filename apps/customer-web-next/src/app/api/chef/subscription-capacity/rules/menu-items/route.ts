...
export async function PUT(request: NextRequest) {
  ...
  const input = parseMenuRuleInput(await request.json().catch(() => null));
  if (!input) return NextResponse.json({ code: "INVALID_MENU_CAPACITY_RULE" }, { status: 400 });
  try {
    const upstream = await authenticatedApiFetch(request, "/chef/subscription-capacity/rules/menu-items", {
