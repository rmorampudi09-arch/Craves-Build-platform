...
function coordinate(value: string | null): number {
  return value !== null && value.length <= 32 && DECIMAL.test(value) ? Number(value) : Number.NaN;
}

function integer(value: string | null, fallback: number): number {
  return value === null ? fallback : value.length <= 10 && /^\d+$/.test(value) ? Number(value) : Number.NaN;
}
...
export async function GET(request: NextRequest) {
  ...
  if (
    ...
    return NextResponse.json(
      { error: "INVALID_LOCATION", message: "Valid latitude, longitude and discovery bounds are required." },
