...
const TERMINAL = new Set<OrderTrackingStatus>(["DELIVERED", "CANCELLED", "CHEF_REJECTED", "REFUNDED", "REFUND_FAILED"]);
...
export default function OrderTrackingPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(response.status === 404 ? "This order could not be found." : response.status === 401 ? "Please sign in again to view tracking." : "Live tracking is temporarily unavailable.");
      }
      setTimeline(body as OrderTrackingTimeline);
    } catch (error) {
      setError(error instanceof Error ? error.message : "Live tracking is temporarily unavailable.");
    } finally {
      setLoading(false);
    }
  }, [orderId]);
    ...
    <main className="min-h-screen bg-[#0B1426] px-4 py-8 text-white sm:px-6">
      <section className="mx-auto max-w-3xl rounded-3xl bg-[#FFF8EC] p-6 text-[#0B1426] shadow-xl sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-amber-700">Order tracking</p>
            <h1 className="mt-2 text-3xl font-bold">{timeline ? LABELS[timeline.currentStatus] : "Live order status"}</h1>
          </div>
          <button type="button" onClick={() => void refresh()} className="rounded-full bg-[#F6B545] px-4 py-2 text-sm font-semibold text-[#0B1426] disabled:opacity-60" disabled={loading}>
            Refresh
          </button>
        </div>

        {loading && !timeline ? <p className="mt-8 text-sm text-slate-600">Loading the latest status…</p> : null}
        {error ? <div role="alert" className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</div> : null}
          ...
          <div className="mt-8 space-y-4">
            {timeline.events.length === 0 ? (
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <p className="mt-1 text-sm text-slate-600">The order is active. More timeline events will appear as its status changes.</p>
              </div>
              ...
              <article key={event.id} className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <time className="mt-1 block text-sm text-slate-500" dateTime={event.occurredAt}>{new Date(event.occurredAt).toLocaleString()}</time>
              </article>
            ))}
          </div>
        ) : null}
