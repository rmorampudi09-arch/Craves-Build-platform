...
export function AdminChefReviewList() {
  ...
  return <section>
    <div className="flex flex-wrap gap-3">{(["PENDING", "APPROVED", "REJECTED"] as AdminChefApplicationStatus[]).map(value => <button key={value} type="button" onClick={() => setStatus(value)} className={`rounded-2xl px-4 py-2 font-bold ${status === value ? "bg-[#6930CA] text-white" : "border border-white/20 text-white"}`}>{value}</button>)}</div>
