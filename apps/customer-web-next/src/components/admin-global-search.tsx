...
function CustomerCaseView({ data }: { data: CustomerCase }) {
  const p = data.profile;
  return <div className="space-y-5">
    <section className="rounded-[26px] bg-[#0b1426] p-6 text-white">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#f6b545]">Customer case</p><h2 className="mt-2 text-2xl font-black">{p.firstName} {p.lastName}</h2><p className="mt-2 text-xs text-slate-300">Full details opened under an audited administrator reason.</p></div><span className="rounded-full bg-emerald-400/15 px-3 py-2 text-xs font-black text-emerald-300">ACTIVE PROFILE</span></div>
    </section>
    ...
    <RelatedOperations identityId={p.identityId}/>
  </div>;
}
...
export function AdminGlobalSearch() {
  ...
  return <div className="space-y-7">
    <section className="overflow-hidden rounded-[32px] bg-[#0b1426] p-6 text-white shadow-[0_28px_80px_-46px_rgba(11,20,38,.8)] sm:p-8"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#f6b545]">Global admin search</p><h1 className="mt-3 max-w-4xl text-3xl font-black sm:text-4xl">Start with what the customer gives you.</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Mobile, email, customer/chef ID or exact name finds the person. Order, payment, refund and delivery references use the operational investigator below. Searches and full case opens require a support reason and create an audit trail.</p>
