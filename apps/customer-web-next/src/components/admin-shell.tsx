...
export function AdminShell() {
  ...
  return <section>
    ...
    <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-5">{links.map(link => <a key={link.href} href={link.href} className="rounded-[28px] border border-white/10 bg-white/5 p-6 text-white transition hover:bg-white/10"><strong className="text-xl">{link.label}</strong><p className="mt-3 text-sm leading-6 text-slate-300">{link.description}</p></a>)}</div>
  </section>;
}
