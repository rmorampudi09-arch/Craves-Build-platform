...
import type { AdminPlanPeriod, AdminPlanStatus, AdminSubscriptionPlan, ApprovedChefReference } from "@/lib/admin-subscription-plan-contract";

const EMPTY = { planCode: "", chefIdentityId: "", name: "", description: "", billingPeriod: "WEEKLY" as AdminPlanPeriod, amount: "", currency: "INR" };
...
export function AdminSubscriptionPlanManager() {
  ...
  async function create(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/admin/subscription-plans", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, chefIdentityId: form.chefIdentityId || null, description: form.description || null, amount, currency: form.currency.toUpperCase() }) });
  ...
  async function updateStatus(planId: string, status: AdminPlanStatus) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Plan status could not be updated.");
      await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Plan status could not be updated."); }
    finally { setBusy(false); }
  }

  return <div className="grid gap-7 lg:grid-cols-[0.8fr_1.2fr]">
    <form onSubmit={create} className="rounded-[30px] bg-[#FFF8EC] p-6 text-slate-950"><h2 className="text-2xl font-bold">Create plan</h2><p className="mt-2 text-sm text-slate-600">Amounts and plan content are operational inputs; this UI does not calculate or recommend them.</p>
      ...
      <label className="mt-4 block text-sm font-bold">Approved chef<select value={form.chefIdentityId} onChange={event => field("chefIdentityId", event.target.value)} className="mt-2 min-h-12 w-full rounded-2xl bg-white px-4"><option value="">Platform-level / unassigned</option>{chefs.map(chef => <option key={chef.identityId} value={chef.identityId}>{chef.displayName} · {chef.email}</option>)}</select></label>
    ...
    <section><div className="flex items-center justify-between gap-3"><h2 className="text-2xl font-bold text-white">Existing plans</h2><button onClick={() => void load()} className="rounded-2xl border border-white/20 px-4 py-2 font-bold text-white">Refresh</button></div>{message && <p className="mt-4 rounded-2xl bg-[#FFF8EC] p-4 text-slate-950" role="status">{message}</p>}<div className="mt-5 space-y-4">{plans.map(plan => <article key={plan.id} className="rounded-[26px] bg-[#FFF8EC] p-6 text-slate-950"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#6930CA]">{plan.status} · {plan.billingPeriod}</p><h3 className="mt-2 text-2xl font-bold">{plan.name}</h3><p className="mt-2 text-sm text-slate-600">{plan.planCode} · {plan.chefIdentityId ? "Chef-assigned" : "Unassigned"}</p></div><strong className="text-xl">{money(plan.amount, plan.currency)}</strong></div><p className="mt-4 text-sm leading-6 text-slate-700">{plan.description ?? "No description"}</p><div className="mt-5 flex flex-wrap gap-2">{(["DRAFT", "ACTIVE", "INACTIVE"] as AdminPlanStatus[]).map(status => <button key={status} disabled={busy || plan.status === status} onClick={() => void updateStatus(plan.id, status)} className="rounded-2xl border border-[#6930CA] px-3 py-2 text-sm font-bold text-[#6930CA] disabled:opacity-40">{status}</button>)}</div></article>)}</div></section>
  </div>;
}
