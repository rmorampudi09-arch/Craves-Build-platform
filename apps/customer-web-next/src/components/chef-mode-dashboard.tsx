...
export function ChefModeDashboard() {
  ...
  if (!identity) {
    return <section className="rounded-[30px] bg-[#FFF8EC] p-7 text-slate-950"><p role="status">{message}</p><a href="/sign-in?returnTo=/chef" className="mt-5 inline-flex rounded-full bg-[#6930CA] px-5 py-3 font-bold text-white">Sign in</a></section>;
  }

  if (!identity.chefEnabled) {
    return <section className="rounded-[30px] bg-[#FFF8EC] p-7 text-slate-950"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#6930CA]">Chef access</p><h2 className="mt-3 text-3xl font-bold">Chef mode is not enabled yet</h2><p className="mt-3 text-sm leading-6 text-slate-600">Submit or review your chef application. Approval and compliance decisions remain with the Craves admin process.</p><a href="/chef/application" className="mt-6 inline-flex rounded-full bg-[#6930CA] px-5 py-3 font-bold text-white">Open chef application</a></section>;
  }

  return <section className="space-y-6"><div className="rounded-[30px] bg-[#FFF8EC] p-7 text-slate-950"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#6930CA]">Chef mode</p><h2 className="mt-3 text-3xl font-bold">Welcome{identity.displayName ? `, ${identity.displayName}` : ""}</h2><p className="mt-3 text-sm text-slate-600">Your backend identity includes the CHEF role. Each service still validates ownership and role on every request.</p></div><div className="grid gap-4 md:grid-cols-2">{approvedLinks.map(link => <a key={link.href} href={link.href} className="rounded-[24px] border border-white/10 bg-white/5 p-5 text-white transition hover:bg-white/10"><strong>{link.label}</strong><p className="mt-2 text-sm leading-6 text-slate-300">{link.description}</p></a>)}</div></section>;
}
