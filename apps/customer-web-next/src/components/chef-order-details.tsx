...
export function ChefOrderDetails({ orderId }: { orderId: string }) {
        ...
        if (!response.ok) {
          throw new Error(
            response.status === 404 || response.status === 403
              ? "This order is not available for your chef account."
              : "Chef order is temporarily unavailable.",
          );
        }
        setOrder(body as ChefOrder);
        setMessage("");
      })
      .catch((error) => {
        if (active) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[30px] border border-[#F6B545]/20 bg-[#FFF8EC] p-6 text-slate-950 shadow-xl shadow-black/10 sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-[#6930CA]">
              {formatChefOrderStatus(order.status)}
            </p>
            <h2 className="mt-2 text-3xl font-bold">
              {order.kitchenName ?? "Chef order"}
            </h2>
            <p className="mt-2 text-sm text-slate-600">
              Updated {new Date(order.updatedAt).toLocaleString("en-IN")}
            </p>
          </div>
          <strong className="rounded-full bg-[#F6B545]/20 px-4 py-2 text-xl">
            {money(order.foodSubtotal, order.currency)}
          </strong>
        </div>
        {order.chefResponseNote && (
          <p className="mt-4 rounded-2xl bg-white p-4 text-sm">
            Chef note: {order.chefResponseNote}
          </p>
        )}
      </section>
      ...
      <section className="rounded-[30px] border border-[#F6B545]/20 bg-[#FFF8EC] p-6 text-slate-950 shadow-xl shadow-black/10">
      ...
      {order.deliveryAddress && (
        <section className="rounded-[30px] border border-[#F6B545]/20 bg-[#FFF8EC] p-6 text-slate-950 shadow-xl shadow-black/10">
          <h2 className="text-2xl font-bold">Delivery recipient</h2>
      ...
      <section className="rounded-[30px] border border-[#F6B545]/20 bg-[#FFF8EC] p-6 text-slate-950 shadow-xl shadow-black/10">
        <h2 className="text-2xl font-bold">Backend totals</h2>
        <dl className="mt-4 grid gap-3 text-sm">
          <div className="flex justify-between">
            <dt>Food subtotal</dt>
            <dd>{money(order.foodSubtotal, order.currency)}</dd>
          </div>
          <div className="flex justify-between">
            <dt>Platform fee</dt>
          ...
          <div className="flex justify-between">
            <dt>Tax</dt>
            <dd>{money(order.taxAmount, order.currency)}</dd>
          </div>
          <div className="flex justify-between">
            <dt>Delivery fee</dt>
            <dd>{money(order.deliveryFee, order.currency)}</dd>
          </div>
          <div className="flex justify-between border-t border-[#F6B545]/40 pt-3 font-bold">
            <dt>Grand total</dt>
