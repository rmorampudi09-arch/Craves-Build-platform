...
export function CustomerAddresses() {
  ...
  function useLocation() {
    if (!navigator.geolocation) {
      setMessage("Browser location is unavailable. Enter coordinates manually.");
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        ...
        const response = await fetch(`/api/customer/addresses/recommendation?${query}`, {
          cache: "no-store",
          credentials: "same-origin",
        });
        ...
        setMessage(
          body.selectedSavedAddress
            ? `This location is near your saved ${body.selectedSavedAddress.addressLabel.toLowerCase()} address.`
            : "Location captured. Complete the postal address before saving.",
        );
      } catch (error) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!input) {
      setMessage("Enter a complete address, valid phone number and map coordinates.");
      return;
    }
    setBusy(true);
    ...
    <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
      <form onSubmit={submit} className="rounded-[30px] bg-[#FFF8EC] p-6 text-slate-950 shadow-2xl shadow-black/20 sm:p-8">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-sm font-semibold">Latitude<input required inputMode="decimal" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3" value={form.latitude} onChange={(event) => setField("latitude", event.target.value === "" ? "" : Number(event.target.value))} /></label>
          <label className="text-sm font-semibold">Longitude<input required inputMode="decimal" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3" value={form.longitude} onChange={(event) => setField("longitude", event.target.value === "" ? "" : Number(event.target.value))} /></label>
        </div>
        ...
        {addresses.map((address) => {
          ...
          return (
            <article key={address.id} className="rounded-[28px] bg-white p-6 text-slate-950 shadow-xl shadow-black/15">
              ...
              <p className="mt-4 text-sm leading-6 text-slate-700">{[address.addressLine1, address.addressLine2, address.landmark, address.areaName, address.city, address.state, address.postalCode].filter(Boolean).join(", ")}</p>
