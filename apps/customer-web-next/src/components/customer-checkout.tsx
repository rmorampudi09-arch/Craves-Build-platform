  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_0.9fr]">
      <form onSubmit={submit} className="rounded-[30px] bg-[#FFF8EC] p-7 text-slate-950 shadow-2xl shadow-black/20">
        ...
        <a href="/addresses" className="mt-2 inline-flex text-sm font-semibold text-[#6930CA]">Manage saved addresses</a>
      ...
      <section className="h-fit rounded-[30px] bg-white p-7 text-slate-950 shadow-2xl shadow-black/15">
        ...
        {checkout && <><dl className="mt-5 space-y-3 text-sm">{[["Food subtotal", checkout.foodSubtotal], ["Platform fee", checkout.platformFee], ["Tax", checkout.taxAmount], ["Delivery fee", checkout.deliveryFee]].map(([label, amount]) => <div key={String(label)} className="flex justify-between"><dt>{label}</dt><dd>{formatMoney(Number(amount), checkout.currency)}</dd></div>)}<div className="flex justify-between border-t border-slate-200 pt-4 text-lg font-bold"><dt>Grand total</dt><dd>{formatMoney(checkout.grandTotal, checkout.currency)}</dd></div></dl><p className="mt-4 text-sm text-slate-600">Checkout reference: {checkout.id}</p><a href={`/checkout/${checkout.id}/payment`} className="mt-6 flex w-full justify-center rounded-full bg-[#6930CA] px-6 py-3 text-sm font-bold text-white">Continue to payment</a><a href={`/checkout/${checkout.id}`} className="mt-3 flex justify-center text-sm font-semibold text-[#6930CA]">View checkout details</a></>}
      </section>
    </div>
  );
}
