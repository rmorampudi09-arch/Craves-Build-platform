...
export function DishCard({
  ...
  const handleQuantityChange = async (change: -1 | 1) => {
    ...
    try {
      ...
      setState("error");
      setMessage(error instanceof Error ? error.message : "This cart quantity could not be updated.");
    }
  };
    ...
    >
      <div className={`relative overflow-hidden bg-[#F1F3F5] ${featured ? "aspect-[16/10]" : "aspect-[4/3]"}`}>
        ...
        <span className="pointer-events-none absolute bottom-3.5 left-3.5 inline-flex items-center gap-1.5 rounded-full bg-white/94 px-2.5 py-1.5 text-[0.65rem] font-black text-[#1A1A1A] shadow-sm backdrop-blur-md">
          <span className={`h-2 w-2 rounded-full ${indicator.dotClass}`} aria-hidden="true" />
          {indicator.label}
        </span>
      ...
      <div className={`flex flex-1 flex-col ${featured ? "p-4.5" : "p-4"}`}>
        <div>
          ...
          <p className="mt-1 truncate text-[0.72rem] font-bold text-[#6B6B6B]">{dish.chef}</p>
        </div>
          ...
          {quantity > 0 ? (
            ...
            >
              <button
                ...
                disabled={state === "busy"}
                className="flex h-10 w-10 items-center justify-center transition hover:bg-[#E5E7EB] disabled:cursor-wait disabled:opacity-50"
            ...
            <button
              ...
              disabled={state === "busy"}
              className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-full bg-[#F1F3F5] px-4 text-xs font-black text-[#1A1A1A] shadow-[0_5px_14px_rgba(26,26,26,0.06)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#E5E7EB] hover:shadow-[0_8px_18px_rgba(26,26,26,0.10)] disabled:cursor-wait disabled:opacity-60"
