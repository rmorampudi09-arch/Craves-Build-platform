...
/** A single dish tile in the browse-foods grid: image, rating, price, "Add to Cart". */
export function DishCard({ dish }: { dish: Dish }) {
  ...
  const handleAdd = () => {
    void addToCart(
      ...
      1,
    ).then(() => setMessage("Added")).catch((error) => setMessage(error instanceof Error ? error.message : "Could not add"));
  };
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-shadow hover:shadow-lg">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        />
        <span
          className={`pointer-events-none absolute bottom-3 left-3 flex h-6 w-6 items-center justify-center rounded border-2 bg-white ${
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="font-display text-lg font-bold leading-tight text-ink">{dish.name}</h3>
        ...
        <p className="mt-2 line-clamp-2 text-xs text-muted-foreground">{dish.desc}</p>
        ...
        <div className="mt-4 flex items-center justify-between">
          <span className="font-display text-xl font-bold text-ink">₹{dish.price}</span>
          <button
            type="button"
            onClick={handleAdd}
          ...
          {message && message !== "Added" && <span className="sr-only" role="alert">{message}</span>}
        </div>
      </div>
    </article>
  );
