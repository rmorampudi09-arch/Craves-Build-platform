        ...
        <div className="relative z-10 flex min-h-[27rem] max-w-[43rem] flex-col justify-center px-6 py-10 sm:px-9 md:min-h-[31rem] md:px-12 lg:min-h-[34rem] lg:px-16">
          ...
          <div className="mt-7 flex flex-wrap items-stretch gap-2.5">
            ...
            <button
              ...
              aria-label={hasDefaultAddress ? `Default delivery address: ${defaultAddressLabel}. Change address` : "Choose a default delivery address"}
            >
              ...
              <span className="min-w-0 max-w-[15rem] sm:max-w-[21rem]">
                <span className="block text-[0.58rem] font-black uppercase tracking-[0.12em] text-[#6B6B6B]">
                  {hasDefaultAddress ? "Default delivery address" : "Set default delivery address"}
                </span>
                <span className="block truncate text-xs font-black text-[#1A1A1A]">
                  {hasDefaultAddress
                    ? `${defaultAddressLabel}${radiusLabel ? ` · ${radiusLabel}` : ""}`
                    : "Choose where Craves should deliver"}
                </span>
              </span>
              <span className="hidden shrink-0 rounded-full bg-[#F1F3F5] px-2.5 py-1 text-[0.62rem] font-black text-[#6B6B6B] sm:inline-flex">
                {hasDefaultAddress ? "Change" : "Add"}
              </span>
