...
export function SubscriptionEnrollmentForm({ planId }: { planId: string }) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      fetch("/api/subscriptions/plans", { cache: "no-store" }).then(response => response.json()),
      ...
      setPlan(selected); setAddresses(activeAddresses);
      const preferred = activeAddresses.find(item => item.isDefault) ?? activeAddresses[0];
      if (preferred) setAddressId(preferred.id);
      setStartDate(today); setMessage(activeAddresses.length ? "" : "Create a saved address before subscribing.");
    }).catch(error => active && setMessage(error instanceof Error ? error.message : "Subscription enrollment is unavailable."));
    return () => { active = false; };
  }, [planId, today]);
  ...
  return <form onSubmit={submit} className="rounded-[30px] bg-[#FFF8EC] p-6 text-slate-950 sm:p-8">
    <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#6930CA]">{plan.billingPeriod}</p><h2 className="mt-2 text-3xl font-bold">{plan.name}</h2><p className="mt-3 text-sm text-slate-600">{plan.description}</p>
