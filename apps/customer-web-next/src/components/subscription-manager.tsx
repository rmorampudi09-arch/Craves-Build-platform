  ...
  return <section>
    ...
    <div className="space-y-5">
      {items.map(item => {
        ...
        return <article key={item.id} className="rounded-[28px] bg-[#FFF8EC] p-6 text-slate-950">
          <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#6930CA]">{item.status.replaceAll("_", " ")}</p><h2 className="mt-2 text-2xl font-bold">Meal subscription</h2><p className="mt-2 text-sm text-slate-600">Starts {new Date(`${item.startDate}T00:00:00Z`).toLocaleDateString("en-IN")}</p></div><div className="flex flex-wrap gap-2">{paymentRequired && <Link href={`/subscriptions/${item.id}/payment`} className="rounded-2xl bg-[#6930CA] px-4 py-2 text-sm font-bold text-white">{item.status === "PAYMENT_FAILED" ? "Retry payment" : "Continue payment"}</Link>}<Link href={`/subscriptions/${item.id}`} className="rounded-2xl border border-[#6930CA] px-4 py-2 text-sm font-bold text-[#6930CA]">View & manage</Link></div></div>
          ...
          {paymentRequired && <p className="mt-4 rounded-2xl bg-white p-3 text-sm font-semibold text-[#6930CA]">This meal plan will not become active until Cashfree sandbox payment is confirmed by the signed backend webhook.</p>}
          <p className="mt-4 text-xs text-slate-500">Pause, resume, skip and cancellation controls are shown in details only when the administrator-approved policy and cutoff permit them.</p>
        </article>;
      })}
    </div>
  </section>;
