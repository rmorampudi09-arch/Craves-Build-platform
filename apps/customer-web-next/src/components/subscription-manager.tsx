  ...
  return <section>
    ...
    <div className="space-y-5">
      {items.map(item => <article key={item.id} className="rounded-[28px] bg-[#FFF8EC] p-6 text-slate-950">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#6930CA]">{item.status.replaceAll("_", " ")}</p><h2 className="mt-2 text-2xl font-bold">Meal subscription</h2><p className="mt-2 text-sm text-slate-600">Starts {new Date(`${item.startDate}T00:00:00Z`).toLocaleDateString("en-IN")}</p></div><a href={`/subscriptions/${item.id}`} className="text-sm font-bold text-[#6930CA]">View details</a></div>
