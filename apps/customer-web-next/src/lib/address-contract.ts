...
export type CustomerAddressInput = Omit<CustomerAddress, "id" | "active" | "createdAt" | "updatedAt">;
...
function text(value: unknown, max: number): string | null {
  if (value === null || value === undefined || value === "" || typeof value !== "string") return null;
...
export function parseCustomerAddresses(value: unknown): CustomerAddress[] | null {
  ...
  const addresses = value.map(parseCustomerAddress);
  return addresses.some(address => address === null) ? null : addresses as CustomerAddress[];
}
...
export function parseLocationRecommendation(value: unknown): LocationRecommendation | null {
  ...
  const latitude = coordinate(raw.latitude, -90, 90);
  const longitude = coordinate(raw.longitude, -180, 180);
  const matchRadiusMeters = typeof raw.matchRadiusMeters === "number" && Number.isInteger(raw.matchRadiusMeters) && raw.matchRadiusMeters >= 1 && raw.matchRadiusMeters <= 100_000 ? raw.matchRadiusMeters : null;
  const distanceMeters = raw.distanceMeters === null || raw.distanceMeters === undefined ? null : typeof raw.distanceMeters === "number" && Number.isInteger(raw.distanceMeters) && raw.distanceMeters >= 0 ? raw.distanceMeters : null;
  const selectedSavedAddress = raw.selectedSavedAddress === null || raw.selectedSavedAddress === undefined ? null : parseCustomerAddress(raw.selectedSavedAddress);
  if (!locationType || !LOCATION_TYPES.has(locationType) || latitude === null || longitude === null || matchRadiusMeters === null || (raw.selectedSavedAddress != null && !selectedSavedAddress) || (raw.distanceMeters != null && distanceMeters === null)) return null;
  return { locationType: locationType as LocationRecommendation["locationType"], latitude, longitude, selectedSavedAddress, distanceMeters, matchRadiusMeters };
}
