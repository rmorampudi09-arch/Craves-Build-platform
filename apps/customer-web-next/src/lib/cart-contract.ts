...
export function parseCart(value: unknown): CustomerCart | null {
  const raw = record(value);
  if (!raw || !Array.isArray(raw.items) || raw.items.length > 200) return null;
  const id = text(raw.id, 64);
  const currency = text(raw.currency, 3);
  const totals = record(raw.totals);
  const foodSubtotal = totals ? money(totals.foodSubtotal) : null;
  const totalCurrency = totals ? text(totals.currency, 3) : null;
...
export function parseAddItemInput(value: unknown): { menuItemId: string; quantity: number } | null {
  const raw = record(value);
  if (!raw) return null;
  const menuItemId = text(raw.menuItemId, 64);
  const quantity = typeof raw.quantity === "number" && Number.isInteger(raw.quantity) && raw.quantity >= 1 && raw.quantity <= 100 ? raw.quantity : null;
  return menuItemId && UUID.test(menuItemId) && quantity !== null ? { menuItemId, quantity } : null;
}

export function parseQuantityInput(value: unknown): { quantity: number } | null {
  const raw = record(value);
  const quantity = raw && typeof raw.quantity === "number" && Number.isInteger(raw.quantity) && raw.quantity >= 1 && raw.quantity <= 100 ? raw.quantity : null;
