import type { ChefKitchen, ChefKitchenInput, EditableKitchenStatus, KitchenStatus } from "./chef-kitchen-types";

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const STATUSES = new Set<KitchenStatus>(["DRAFT", "ACTIVE", "INACTIVE", "SUSPENDED"]);
const EDITABLE = new Set<EditableKitchenStatus>(["DRAFT", "ACTIVE", "INACTIVE"]);
...
export function parseChefKitchen(value: unknown): ChefKitchen | null {
  ...
  if (!id || !UUID.test(id) || !kitchenName || !addressLine1 || !city || !state || !status || !STATUSES.has(status) || !createdAt || !updatedAt) return null;
  return {
    id,
    kitchenName,
...
export function parseChefKitchenInput(value: unknown): ChefKitchenInput | null {
  ...
  const hasLon = raw.longitude !== null && raw.longitude !== undefined && raw.longitude !== "";
  if (!kitchenName || !addressLine1 || !city || !state || !status || !EDITABLE.has(status) || hasLat !== hasLon || (hasLat && (latitude === null || longitude === null))) return null;
  return {
    kitchenName,
    displayName: optional(raw.displayName, 180),
    description: optional(raw.description, 2000),
