...
export type NearbyKitchenDiscovery = {
  latitude: number;
  longitude: number;
  radiusMeters: number;
  page: PageMetadata;
...
export type NearbyMenuDiscovery = {
  latitude: number;
  longitude: number;
  radiusMeters: number;
  page: PageMetadata;
...
function parsePage(value: unknown): PageMetadata | null {
  ...
  if (!page) return null;
  const pageNumber = integer(page.page, 0, 1_000_000);
  const size = integer(page.size, 1, 100);
  const totalElements = integer(page.totalElements, 0, Number.MAX_SAFE_INTEGER);
  const totalPages = integer(page.totalPages, 0, Number.MAX_SAFE_INTEGER);
  if (pageNumber === null || size === null || totalElements === null || totalPages === null || typeof page.hasNext !== "boolean") return null;
  return { page: pageNumber, size, totalElements, totalPages, hasNext: page.hasNext };
}
...
function parseKitchen(value: unknown): NearbyKitchen | null {
  ...
  const distanceMeters = integer(raw.distanceMeters, 0, 10_000_000);
  const activeMenuItemCount = integer(raw.activeMenuItemCount, 0, 100_000);
  ...
  return {
    ...
    areaName: text(raw.areaName, 120),
    city,
    state,
    distanceMeters,
    activeMenuItemCount
...
function parseMenuItem(value: unknown): NearbyMenuItem | null {
  ...
  const foodType = text(raw.foodType, 20);
  const city = text(raw.city, 120);
  const state = text(raw.state, 120);
  const currency = text(raw.currency, 3);
  const price = number(raw.price, 0, 10_000_000);
  const distanceMeters = integer(raw.distanceMeters, 0, 10_000_000);
  ...
  return {
    ...
    kitchenId,
    kitchenName,
    kitchenDisplayName: text(raw.kitchenDisplayName, 180),
    areaName: text(raw.areaName, 120),
    city,
    state,
    distanceMeters,
    ...
    description: text(raw.description, 1_000),
    category,
    foodType: foodType as NearbyMenuItem["foodType"],
    price,
    currency: currency.toUpperCase(),
    servesCount: integer(raw.servesCount, 1, 100),
    preparationTimeMinutes: integer(raw.preparationTimeMinutes, 1, 1_440),
    spiceLevel: spice && SPICE_LEVELS.has(spice) ? spice as NearbyMenuItem["spiceLevel"] : null,
    primaryImageUrl: httpsUrl(raw.primaryImageUrl)
  };
}

export function parseKitchenDiscovery(value: unknown): NearbyKitchenDiscovery | null {
  const raw = object(value);
  if (!raw || !Array.isArray(raw.kitchens) || raw.kitchens.length > 100) return null;
  const latitude = number(raw.latitude, -90, 90);
  const longitude = number(raw.longitude, -180, 180);
  const radiusMeters = integer(raw.radiusMeters, 1, 100_000);
  const page = parsePage(raw.page);
  const kitchens = raw.kitchens.map(parseKitchen);
  if (latitude === null || longitude === null || radiusMeters === null || !page || kitchens.some(item => item === null)) return null;
  return { latitude, longitude, radiusMeters, page, kitchens: kitchens as NearbyKitchen[] };
}

export function parseMenuDiscovery(value: unknown): NearbyMenuDiscovery | null {
  const raw = object(value);
  if (!raw || !Array.isArray(raw.menuItems) || raw.menuItems.length > 100) return null;
  const latitude = number(raw.latitude, -90, 90);
  const longitude = number(raw.longitude, -180, 180);
  const radiusMeters = integer(raw.radiusMeters, 1, 100_000);
  const page = parsePage(raw.page);
  const menuItems = raw.menuItems.map(parseMenuItem);
  if (latitude === null || longitude === null || radiusMeters === null || !page || menuItems.some(item => item === null)) return null;
  return { latitude, longitude, radiusMeters, page, menuItems: menuItems as NearbyMenuItem[] };
}
