...
const reply = (body: unknown, status: number) => NextResponse.json(body, { status, headers: PRIVATE_HEADERS });
...
export async function emailVerificationBff(request: NextRequest, action?: EmailVerificationAction): Promise<NextResponse> {
  ...
  if (action) {
    ...
    request = bounded;
    const parsed = emailVerificationRequests[action].safeParse(await request.json().catch(() => null));
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (typeof code === "string" && Object.hasOwn(emailVerificationErrors, code)) {
        ...
        if (response.status === safe.status) return reply({ code, message: safe.message }, safe.status);
      }
      ...
      return reply({ code: "EMAIL_VERIFICATION_UNAVAILABLE" }, response.status === 429 ? 429 : 503);
    }
