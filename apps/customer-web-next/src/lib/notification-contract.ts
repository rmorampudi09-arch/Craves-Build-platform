...
export function parseCustomerNotification(value: unknown): CustomerNotification | null {
  ...
  const id = text(notice.id, 64);
  const title = text(notice.title, 180);
  const body = text(notice.body, 1_500);
  const noticeType = text(notice.noticeType, 80);
  const createdAt = instant(notice.createdAt);
  const readAt = instant(notice.readAt);
  ...
  return {
    ...
    body,
    noticeType,
    targetType: text(notice.targetType, 80),
    targetId,
    readAt,
    createdAt
  };
...
export function parseCustomerNotifications(value: unknown): CustomerNotification[] | null {
  ...
  const notices = value.map(parseCustomerNotification);
