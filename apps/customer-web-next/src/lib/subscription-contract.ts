...
export function parsePublicSubscriptionPlan(value: unknown): PublicSubscriptionPlan | null {
  ...
  const planCode = text(raw.planCode, 80);
  const name = text(raw.name, 160);
  const billingPeriod = text(raw.billingPeriod, 20);
...
export function parsePublicSubscriptionPlans(value: unknown): PublicSubscriptionPlan[] | null {
  ...
  const parsed = value.map(parsePublicSubscriptionPlan);
  return parsed.some(item => item === null) ? null : parsed as PublicSubscriptionPlan[];
}
...
export function parseCustomerSubscription(value: unknown): CustomerSubscription | null {
  ...
  const raw = value as Record<string, unknown>;
  const id = text(raw.id, 64);
  const planId = text(raw.planId, 64);
  ...
  return {
    ...
    startDate,
    endDate: raw.endDate === null || raw.endDate === undefined ? null : dateOnly(raw.endDate),
    nextServiceDate: raw.nextServiceDate === null || raw.nextServiceDate === undefined ? null : dateOnly(raw.nextServiceDate),
    deliveryAddressId,
...
export function parseCustomerSubscriptions(value: unknown): CustomerSubscription[] | null {
  if (!Array.isArray(value) || value.length > 500) return null;
  const parsed = value.map(parseCustomerSubscription);
  return parsed.some(item => item === null) ? null : parsed as CustomerSubscription[];
}
