...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!identity) return NextResponse.json({ code: "INVALID_IDENTITY_RESPONSE" }, { status: 502, headers: { "Cache-Control": "no-store" } });
    if (!identity.adminEnabled) return NextResponse.json({ code: "ADMIN_ACCESS_REQUIRED" }, { status: 403, headers: { "Cache-Control": "no-store" } });
    return NextResponse.json(identity, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    if (error instanceof SessionRequiredError) {
      return NextResponse.json({ code: "AUTHENTICATION_REQUIRED" }, { status: 401, headers: { "Cache-Control": "no-store" } });
    }
