...
No new secrets, permissions, DNS, resources or store settings. Use the already-authorized Azure service connection; if any scoped read is unavailable, stop and report it rather than grant broader permissions. Subsequent to web rollout39106, the owner confirmed receiving and successfully entering a Craves email code today; the fresh profile showed verified status, including after Refresh verification status. This is separate user/browser evidence, not a conclusion inferred from aggregate counts. Cross-service identity/version correlation and the full chef-entry criteria remain separate. Retain the receipt and never mark F05/F22 accepted from counts alone.
