...
API_PATH="${API_PATH:-api/v1/admin/internal-access}"
API_VERSION="${API_VERSION:-2022-08-01}"
CONFIRM_APIM_WRITE="${CONFIRM_APIM_WRITE:-false}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
POLICY_TEMPLATE="$ROOT/infra/apim/internal-admin-rbac/authenticated-policy.xml"
...
BACKEND="https://${AUTH_FQDN}/api/v1/admin/internal-access"
...
if (( ${#PATH_OWNERS[@]} == 0 )); then
  az apim api create -g "$RG" --service-name "$APIM" --api-id "$API_ID" \
    --display-name "Craves Internal Administrator Access" --path "$API_PATH" \
...
put_operation "get-internal-admin-role-catalog" "GET" "/roles" "List internal administrator roles"
...
verify_operation() {
  ...
  [[ "$ACTUAL_METHOD" == "$EXPECTED_METHOD" ]] || fail "$ID method read-back failed"
  [[ "$ACTUAL_TEMPLATE" == "$EXPECTED_TEMPLATE" ]] || fail "$ID URL template read-back failed"
...
verify_operation "get-internal-admin-role-catalog" "GET" "/roles"
...
HTTP_STATUS=$(curl --silent --output /dev/null --write-out '%{http_code}' --max-time 30 "${GATEWAY_URL%/}/${API_PATH}/roles")
[[ "$HTTP_STATUS" == "401" ]] || fail "Unauthenticated gateway guard returned HTTP $HTTP_STATUS instead of 401"

echo "SUCCESS: Internal administrator RBAC APIM operations configured and verified."
