...
put_operation "$SUB_PAYMENT_API" "$SUBSCRIPTION_PAYMENT_BACKEND" "$AUTH_POLICY" "__SUBSCRIPTION_PAYMENT_BACKEND_URL__" \
  "create-subscription-payment-order" "POST" "/invoices/{invoiceId}/orders" "Create subscription payment provider order" "$INVOICE_PARAM" true
put_operation "$SUB_PAYMENT_API" "$SUBSCRIPTION_PAYMENT_BACKEND" "$AUTH_POLICY" "__SUBSCRIPTION_PAYMENT_BACKEND_URL__" \
  "verify-subscription-razorpay-payment" "POST" "/invoices/{invoiceId}/verify" "Verify Razorpay subscription payment" "$INVOICE_PARAM" true
...
echo "SUCCESS: Subscription payment provider routes and Cashfree webhook APIM operations configured and verified."
