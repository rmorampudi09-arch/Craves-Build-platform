...
EXPECTED_TEMPLATE="/{orderId}/timeline"
...
if ! az apim api operation show -g "$RG" --service-name "$APIM" --api-id "$API_ID" --operation-id "$OPERATION_ID" >/dev/null 2>&1; then
  echo "Order timeline operation is already absent."
  exit 0
fi
...
echo "SUCCESS: Removed only ${OPERATION_ID}; existing Order API and other operations were preserved."
