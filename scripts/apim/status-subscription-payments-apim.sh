...
verify_route "$SUB_PAYMENT_API" "$SUBSCRIPTION_PAYMENT_BACKEND" POST "/invoices/{invoiceId}/verify" true
verify_route "$PAYMENT_API" "$PAYMENT_BACKEND" POST "/webhooks/cashfree" false

echo "SUCCESS: Subscription payment provider routes and Cashfree webhook APIM status are valid."
