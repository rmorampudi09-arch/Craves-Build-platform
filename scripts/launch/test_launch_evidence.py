...
class LaunchEvidenceTest(unittest.TestCase):
    ...
    def junit(self, name="RequiredTest", tests=2, failures=0, errors=0, skipped=0, file="TEST-required.xml"):
        ...
        cases = ''.join(f'<testcase name="case{i}">' + (f'<{outcomes[i]}/>' if i < len(outcomes) else '') + '</testcase>' for i in range(tests))
        (self.reports / file).write_text(f'<testsuite name="test.{name}" tests="{tests}" failures="{failures}" errors="{errors}" skipped="{skipped}">{cases}</testsuite>')

    def test_declared_count_cannot_hide_missing_case_nodes(self):
        (self.reports / 'TEST-required.xml').write_text('<testsuite name="test.RequiredTest" tests="2"/>')
        self.assertEqual('RED', self.java()['status'])

    def test_zero_summary_cannot_hide_skipped_child(self):
        self.junit()
        report = self.reports / 'TEST-required.xml'
        report.write_text(report.read_text().replace('<testcase name="case0">', '<testcase name="case0"><skipped/>'))
        self.assertEqual('RED', self.java()['status'])
