...
wait_ready() {
  ...
  local attempt current_image latest_revision ready_revision running_status
  ...
  for ((attempt=1; attempt<=attempts; attempt++)); do
    ...
    echo "Waiting for $app_name ($attempt/$attempts): running=$running_status latest=$latest_revision ready=$ready_revision" >&2
    sleep "$sleep_seconds"
  done
  return 1
}
...
rollback_updated_services() {
  ...
  for ((index=${#UPDATED_KEYS[@]}-1; index>=0; index--)); do
    ...
    if az containerapp update -g "$RESOURCE_GROUP" -n "$app" \
      --image "$previous_image" --only-show-errors >/dev/null; then
      if rollback_revision=$(wait_ready "$app" "$previous_image" 48 10); then
...
while IFS= read -r service; do
  ...
  az containerapp update -g "$RESOURCE_GROUP" -n "$app" \
    --image "$target_image" --only-show-errors >/dev/null \
    || abort_release "Container App update failed for $app."

  new_revision=$(wait_ready "$app" "$target_image" 48 10) \
