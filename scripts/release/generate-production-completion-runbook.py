...
def main() -> None:
    ...
    lines: list[str] = [
        ...
        "6. Run seven-service deployment only with all execution/provider/worker/enforcement flags forced false.",
        "7. Run customer-web deployment only to the existing Container App target; do not create billable resources implicitly.",
        "8. Run APIM pipelines one module at a time after owning services are healthy.",
        "9. Complete mobile native shell, Firebase secure files and signing prerequisites before Android/iOS builds.",
        "10. Activate runtime features downstream-first in separate controlled sessions.",
        "11. Rotate existing Storage/PostgreSQL credentials only during the final production-completion security phase.",
        "",
    ...
    lines.extend(
        [
            ...
            "- Existing legacy bindings and existing credential rotation remain intentionally deferred.",
            "",
            "## Pipeline catalog",
            "",
        ]
