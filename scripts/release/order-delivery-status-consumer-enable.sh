...
[[ -n "$PREVIOUS_LATEST" ]] || fail 'Order Container App latest revision is missing.'
...
SUB_CREATED=false
if ! az servicebus topic subscription show --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --name "$SUB" --output none --only-show-errors 2>/dev/null; then
...
SUB_JSON=$(az servicebus topic subscription show --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --name "$SUB" --output json --only-show-errors)
PREVIOUS_SUB_STATUS=$(jq -r '.status // "Active"' <<<"$SUB_JSON")

if ! az servicebus topic subscription rule show --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --subscription-name "$SUB" --name "$RULE" --output none --only-show-errors 2>/dev/null; then
...
RULE_JSON=$(az servicebus topic subscription rule show --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --subscription-name "$SUB" --name "$RULE" --output json --only-show-errors)
...
az servicebus topic subscription rule delete --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --subscription-name "$SUB" --name '$Default' --output none --only-show-errors 2>/dev/null || true
RULE_COUNT=$(az servicebus topic subscription rule list --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --subscription-name "$SUB" --query 'length(@)' --output tsv --only-show-errors)
...
SUBSCRIPTION_SCOPE=$(az servicebus topic subscription show --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --name "$SUB" --query id --output tsv --only-show-errors)
...
if [[ "${RECEIVER_COUNT:-0}" == '0' ]]; then
  az servicebus topic subscription update --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --name "$SUB" --status Disabled --output none --only-show-errors || true
...
rollback_order() {
  ...
  if (( ${#set_args[@]} > 0 )); then
    az containerapp update --resource-group "$RG" --name "$APP" --set-env-vars "${set_args[@]}" --output none --only-show-errors || true
  fi
  if (( ${#remove_args[@]} > 0 )); then
    az containerapp update --resource-group "$RG" --name "$APP" --remove-env-vars "${remove_args[@]}" --output none --only-show-errors || true
  fi
  az servicebus topic subscription update --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --name "$SUB" --status "$PREVIOUS_SUB_STATUS" --output none --only-show-errors || true
}
...
az servicebus topic subscription update --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --name "$SUB" --status Active --output none --only-show-errors
...
for ATTEMPT in $(seq 1 60); do
  ...
  HEALTH=''
  if [[ -n "$LATEST" ]]; then
    HEALTH=$(az containerapp revision show --resource-group "$RG" --name "$APP" --revision "$LATEST" --query properties.healthState --output tsv --only-show-errors 2>/dev/null || true)
  fi
  echo "Attempt $ATTEMPT/60: latest=$LATEST ready=$READY_REV health=$HEALTH running=$RUNNING deliveryStatusConsumer=$ENABLED"

  REVISION_OK=false
  if [[ "$ALREADY_CONFIGURED" == true || "$LATEST" != "$PREVIOUS_LATEST" ]]; then REVISION_OK=true; fi
  if [[ "$REVISION_OK" == true && -n "$LATEST" && "$LATEST" == "$READY_REV" && "$HEALTH" == 'Healthy' && "$RUNNING" == 'Running' && "${ENABLED,,}" == 'true' ]]; then
    READY=true
    break
  fi
  if [[ "$HEALTH" == 'Unhealthy' || "$RUNNING" == 'Failed' ]]; then break; fi
  sleep 10
done
...
[[ "${$(value_of "$FINAL" CRAVES_DELIVERY_STATUS_CONSUMER_ENABLED),,}" == 'true' ]] 2>/dev/null || true
...
SUB_FINAL=$(az servicebus topic subscription show --resource-group "$RG" --namespace-name "$NS" --topic-name "$TOPIC" --name "$SUB" --output json --only-show-errors)
[[ "$(jq -r '.status // ""' <<<"$SUB_FINAL")" == 'Active' ]] || { rollback_order; fail 'Service Bus subscription is not Active.'; }
