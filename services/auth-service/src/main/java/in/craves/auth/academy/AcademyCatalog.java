...
public class AcademyCatalog {
    ...
    private int gradedQuestionsPerLesson;
    ...
    public AcademyCatalog(ObjectMapper mapper) throws IOException {
        ...
        // Validate every source question, including those used as ungraded checks.
        validateCatalog();
    ...
    private void applyEngineeringCurriculum(ObjectMapper mapper) throws IOException {
        ...
        for (String field : java.util.List.of("version", "sourceRevision", "reviewedOn", "evidenceLabel")) {
            catalog.put(field, index.path(field).asText());
        }
        if (index.has("gradedQuestionsPerLesson")) {
            if (!index.path("gradedQuestionsPerLesson").isInt() || index.path("gradedQuestionsPerLesson").asInt() != 2) {
                throw new IllegalStateException("Unsupported deployed Academy assessment contract");
            }
            gradedQuestionsPerLesson = 2;
        }
    ...
    /** Preserve the existing two-question UI without discarding extra teaching.
     *  This opt-in content projection changes neither scoring nor XP semantics.
     */
    private void applyAssessmentPolicy() {
        if (gradedQuestionsPerLesson == 0) return;
        for (JsonNode course : catalog.path("courses")) for (JsonNode lesson : course.path("lessons")) {
            ArrayNode questions = ((ObjectNode) lesson).withArray("questions");
            if (questions.size() < gradedQuestionsPerLesson || questions.size() > gradedQuestionsPerLesson + 1) {
                throw new IllegalStateException("Unexpected Academy question-bank size");
            }
            if (questions.size() > gradedQuestionsPerLesson) {
                JsonNode check = questions.remove(gradedQuestionsPerLesson);
                requireText(check, "prompt");
                requireText(check, "explanation");
                ((ObjectNode) lesson).withArray("steps").addObject()
                    .put("label", "Reasoning checkpoint (ungraded)")
                    .put("detail", check.path("prompt").asText()
                        + " Explain your answer before reading the resolution. Resolution: "
                        + check.path("options").get(check.path("answer").asInt()).asText()
                        + ". " + check.path("explanation").asText());
            }
        }
    }
    ...
    private void validateCatalog() {
        ...
        for (JsonNode course : catalog.path("courses")) {
            ...
            for (JsonNode lesson : course.path("lessons")) {
                ...
                for (JsonNode question : lesson.path("questions")) {
                    ...
                    if (!question.path("answer").isInt()) throw new IllegalStateException("Invalid academy answer type");
        ...
        for (JsonNode course : catalog.path("courses")) {
            ...
            for (JsonNode prerequisite : course.path("prerequisites")) {
                ...
                if (required.equals(courseId) || !courseIds.contains(required)) throw new IllegalStateException("Invalid academy prerequisite");
            }
        }
    }
    ...
    public JsonNode publicCatalog() {
        ...
        for (JsonNode course : copy.path("courses")) for (JsonNode lesson : course.path("lessons")) {
            for (JsonNode question : lesson.path("questions")) {
