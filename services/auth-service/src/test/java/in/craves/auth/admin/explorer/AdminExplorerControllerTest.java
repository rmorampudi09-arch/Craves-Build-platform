package in.craves.auth.admin.explorer;
...
class AdminExplorerControllerTest {
 Authentication actor(String role){Authentication a=mock(Authentication.class);when(a.isAuthenticated()).thenReturn(true);when(a.getPrincipal()).thenReturn(new CurrentUser(UUID.randomUUID(),"test-uid","+919999999999",List.of(role),1L));return a;}
