...
class ReferralSourceOutboxDatabaseTest {
 ...
 long count(){return db.queryForObject("SELECT count(*) FROM auth_schema.referral_source_outbox",Long.class);}
 String status(UUID id){return db.queryForObject("SELECT status FROM auth_schema.referral_source_outbox WHERE event_id=?",String.class,id);}
 ...
 @Test void persistedPayloadCannotBeChangedOrDeleted(){
  UUID id=enqueue("a");assertThrows(Exception.class,()->db.update("UPDATE auth_schema.referral_source_outbox SET envelope='{}' WHERE event_id=?",id));
  assertThrows(Exception.class,()->db.update("DELETE FROM auth_schema.referral_source_outbox WHERE event_id=?",id));
  assertThrows(Exception.class,()->db.execute("TRUNCATE auth_schema.referral_source_outbox"));assertEquals(1,count());
 }
 @Test void staleLeaseCannotAcknowledgeNewOwner(){
  ...
  db.update("UPDATE auth_schema.referral_source_outbox SET lease_until=now()-interval '1 second' WHERE event_id=?",id);
 ...
 @Test void rejectionIsDeadAndExhaustedCrashIsRecovered(){
  ...
  UUID b=enqueue("b");outbox.claim();db.update("UPDATE auth_schema.referral_source_outbox SET attempts=40,lease_until=now()-interval '1 second' WHERE event_id=?",b);
