...
public class AdminIntegrationRecoveryController {
    ...
    private static CravesPrincipal requireReadAccess(Authentication authentication, RecoverySource source) {
        ...
        boolean allowed = source.paymentSource()
            ? principal.hasAnyRole("PLATFORM_ADMIN", "SUPPORT_ADMIN", "PAYMENTS_ADMIN", "AUDIT_ADMIN")
            : principal.hasAnyRole("PLATFORM_ADMIN", "SUPPORT_ADMIN", "OPERATIONS_ADMIN", "AUDIT_ADMIN");
        if (!allowed) {
    ...
    private static CravesPrincipal requireReplayAccess(Authentication authentication, RecoverySource source) {
        CravesPrincipal principal = requirePrincipal(authentication);
        boolean allowed = source.paymentSource()
            ? principal.hasAnyRole("PLATFORM_ADMIN", "PAYMENTS_ADMIN")
            : principal.hasAnyRole("PLATFORM_ADMIN", "OPERATIONS_ADMIN");
        if (!allowed) {
    ...
    private static UUID correlationId(String value) {
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "X-Correlation-ID must be a UUID for admin recovery");
        }
    }
}
