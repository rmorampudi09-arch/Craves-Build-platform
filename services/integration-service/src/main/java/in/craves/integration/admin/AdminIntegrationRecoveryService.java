...
public class AdminIntegrationRecoveryService {
    ...
    public ReplayResponse replay(
        ...
        int updated = switch (source) {
            case CASHFREE -> jdbcTemplate.update("""
                ...
                """, id);
            case RAZORPAY -> jdbcTemplate.update("""
    ...
    private DeadLetterSnapshot load(RecoverySource source, UUID id, boolean lock) {
        ...
        return switch (source) {
            ...
            case RAZORPAY -> jdbcTemplate.query(
                """
                SELECT id,
                       'razorpay' AS provider_id,
                       event_identity,
                       processing_status,
                       attempt_count,
                       next_attempt_at,
    ...
    private static DeadLetterSnapshot snapshot(
        RecoverySource source,
        UUID id,
        String providerId,
    ...
    private void audit(
        ...
        jdbcTemplate.update(
            ...
            actorIdentityId,
            switch (source) {
                case CASHFREE -> "CASHFREE_WEBHOOK";
                case RAZORPAY -> "RAZORPAY_WEBHOOK";
                case DELIVERY -> "DELIVERY_WEBHOOK";
            },
            resourceId,
            action,
            reason,
        ...
        public static RecoverySource parse(String value) {
            try {
                ...
                throw new ResponseStatusException(
                    ...
                    "Recovery source must be CASHFREE, RAZORPAY or DELIVERY"
                );
            }
        }

        public boolean paymentSource() {
            return this == CASHFREE || this == RAZORPAY;
        }
    }
