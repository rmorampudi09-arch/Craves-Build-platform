...
public class ShiprocketPickupProvisioningService {
    ...
    public ProvisioningResult provision(PickupLocationProvisioningRequest request) {
        ...
        try {
            ...
            // A network/5xx failure may occur after the provider committed the pickup. Never retry
            // the mutation blindly: reconcile the deterministic provider code first.
    ...
    private void verifyProviderLocation(JsonNode provider, ValidatedPickup expected, String externalCode) {
    ...
    private void persistMapping(ValidatedPickup pickup, String externalCode, JsonNode providerLocation, boolean created) {
        ...
        metadata.put("kitchen_id", pickup.kitchenId().toString());
