...
public final class ChefReferralEarningsMirror {
    ...
    public void apply(UUID event,JsonNode body) {
        ...
        lock("chef-referral-order:"+order); lock("chef-referral-owner:"+owner);
        ...
        if(amount>0) {
            ...
            long food=((BigDecimal)source.getFirst().get("gross")).movePointRight(2).longValueExact();
            ...
            var rows=db.queryForList("SELECT * FROM payment_schema.chef_referral_posting WHERE posting_id=? AND amount_paise>0",original);
            if(rows.size()!=1) throw new IllegalStateException("ORIGINAL_CHEF_POSTING_NOT_RECEIVED");
            var credit=rows.getFirst();
            if(!owner.equals(credit.get("beneficiary_id")) || !order.equals(credit.get("chef_order_id")) || !seller.equals(credit.get("selling_chef_id"))
