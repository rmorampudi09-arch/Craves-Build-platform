...
public final class ReferralOutbox {
    ...
    public UUID enqueue(String key,String type,UUID aggregate,Instant occurred,JsonNode payload){
        ...
        var previous=db.queryForList("SELECT event_id,content_hash FROM payment_schema.referral_source_outbox WHERE event_key=?",key);
        if(!previous.isEmpty()){
        ...
        db.update("INSERT INTO payment_schema.referral_source_outbox(event_id,event_key,aggregate_id,content_hash,envelope) VALUES (?,?,?,?,?)",id,key,aggregate,hash,body);
        return id;
    }
    public Work claim(){return tx.execute(s->{
        UUID lease=UUID.randomUUID();
        var rows=db.query("WITH due AS (SELECT event_id FROM payment_schema.referral_source_outbox WHERE attempts<40 AND ((status='PENDING' AND next_attempt_at<=now()) OR (status='SENDING' AND lease_until<=now())) ORDER BY next_attempt_at,event_id LIMIT 1 FOR UPDATE SKIP LOCKED) UPDATE payment_schema.referral_source_outbox o SET status='SENDING',lease_id=?,lease_until=now()+interval '60 seconds',attempts=attempts+1 FROM due WHERE o.event_id=due.event_id RETURNING o.event_id,o.envelope,o.attempts",(rs,n)->new Work(rs.getObject(1,UUID.class),lease,rs.getString(2),rs.getInt(3)),lease);
        db.update("UPDATE payment_schema.referral_source_outbox SET status='DEAD',last_code='ATTEMPTS_EXHAUSTED',lease_id=NULL,lease_until=NULL WHERE status='SENDING' AND attempts>=40 AND lease_until<=now()");
    ...
    public void acknowledge(Work work,JsonNode reply){
        ...
        db.update("UPDATE payment_schema.referral_source_outbox SET status='RECEIVED',last_code='DURABLE_RECEIPT',lease_id=NULL,lease_until=NULL WHERE event_id=? AND status='SENDING' AND lease_id=? AND lease_until>now()",work.eventId(),work.lease());
    }
