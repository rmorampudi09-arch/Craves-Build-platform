...
CREATE TABLE payment_schema.referral_source_outbox(
...
CREATE INDEX referral_source_due ON payment_schema.referral_source_outbox(next_attempt_at,event_id) WHERE status IN ('PENDING','SENDING');
CREATE FUNCTION payment_schema.guard_referral_source_history() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN
 ...
 RETURN NEW;END;$$;
CREATE TRIGGER referral_source_history BEFORE UPDATE OR DELETE ON payment_schema.referral_source_outbox FOR EACH ROW EXECUTE FUNCTION payment_schema.guard_referral_source_history();
CREATE TRIGGER referral_source_no_truncate BEFORE TRUNCATE ON payment_schema.referral_source_outbox FOR EACH STATEMENT EXECUTE FUNCTION payment_schema.guard_referral_source_history();
