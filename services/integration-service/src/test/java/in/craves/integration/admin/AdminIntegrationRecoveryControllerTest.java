...
class AdminIntegrationRecoveryControllerTest {
    @Test
    void supportAdminCannotReplayPaymentDeadLetter() {
    ...
    @Test
    void paymentsAdminCanReplayRazorpayDeadLetter() {
        AdminIntegrationRecoveryService service = mock(AdminIntegrationRecoveryService.class);
        AdminIntegrationRecoveryController controller = new AdminIntegrationRecoveryController(service);
        ...
        var authentication = new UsernamePasswordAuthenticationToken(principal, null, Set.of());
        when(service.replay(eq(RecoverySource.RAZORPAY), eq(item), eq(actor), any(), eq(correlation)))
            .thenReturn(new ReplayResponse(
                RecoverySource.RAZORPAY,
                item,
                "RECEIVED",
                0,
                correlation,
        ...
        controller.replay(
            ...
            "razorpay",
            item,
            "Replay approved after Razorpay provider recovery",
            correlation.toString()
        );

        verify(service).replay(
            eq(RecoverySource.RAZORPAY),
            eq(item),
            eq(actor),
            eq("Replay approved after Razorpay provider recovery"),
            eq(correlation)
        );
    }

    @Test
    void paymentsAdminCanStillReplayOptionalCashfreeDeadLetter() {
        AdminIntegrationRecoveryService service = mock(AdminIntegrationRecoveryService.class);
        AdminIntegrationRecoveryController controller = new AdminIntegrationRecoveryController(service);
        UUID actor = UUID.randomUUID();
        UUID item = UUID.randomUUID();
        ...
        controller.replay(
            ...
            "cashfree",
            item,
            "Replay approved after Cashfree provider recovery",
            correlation.toString()
        );
        ...
        verify(service).replay(
            ...
            eq(item),
            eq(actor),
            eq("Replay approved after Cashfree provider recovery"),
            eq(correlation)
        );
    }
}
