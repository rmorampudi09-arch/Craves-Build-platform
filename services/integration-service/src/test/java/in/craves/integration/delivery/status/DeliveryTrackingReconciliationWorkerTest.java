...
class DeliveryTrackingReconciliationWorkerTest {
    ...
    void recoversFinalLeasesThenPerformsReadOnlyProviderTracking() {
        ...
        DeliveryLeaseRecoveryRepository leaseRecovery = mock(DeliveryLeaseRecoveryRepository.class);
        DeliveryStatusUpdateService statusService = mock(DeliveryStatusUpdateService.class);
        DeliveryTelemetryPublisherService telemetryService = mock(DeliveryTelemetryPublisherService.class);
        ...
        TrackingWorkItem workItem = new TrackingWorkItem(
            UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), "borzo", "1250032", 1
        );
        TrackingSnapshot snapshot = new TrackingSnapshot(
            new ProviderDelivery(
                "borzo", "1250032", "50032", DeliveryStatus.IN_TRANSIT, "active",
                new BigDecimal("125.50"), new BigDecimal("125.50"),
                "https://tracking.example/1", new ObjectMapper().createObjectNode(),
        ...
        when(statusService.processTracking(workItem, snapshot)).thenReturn(
            new DeliveryStatusUpdateService.ProcessingResult(workItem.deliveryJobId(), true, false, "APPLIED")
        );
        when(telemetryService.capture(workItem, snapshot)).thenReturn(
            new DeliveryTelemetryPublisherService.CaptureResult(false, "NO_PROVIDER_TELEMETRY")
        );

        DeliveryTrackingReconciliationWorker worker = new DeliveryTrackingReconciliationWorker(
            List.of(adapter), repository, leaseRecovery, statusService, telemetryService, properties
        );
        ...
        verify(telemetryService).capture(workItem, snapshot);
    }
}
