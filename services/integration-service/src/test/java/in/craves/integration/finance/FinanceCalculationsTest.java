...
class FinanceCalculationsTest {
    ...
    @Test void dueAtIs48ElapsedHoursFromDeliveryNotBankTPlusTwo() {
        ...
        assertEquals(delivered.plusSeconds(172800),p.automaticDueAt(delivered));
        assertEquals(delivered,p.manualAvailableAt(delivered));
    }
    ...
    @Test void subscriptionUsesExplicitOccurrencesAndOnePlatformFee() {
        ...
        assertEquals("924.08",result.total());
        assertEquals("1.50",result.occurrences().getFirst().platformAllocation());
        assertEquals("343.17",result.occurrences().getFirst().chef().payable());
        assertTrue(result.breakdownRequiredBeforePayment());assertTrue(result.simulation());
    }
    ...
    void sixtySecondBoundaryIsServerEnforced(int seconds,boolean expected) {
        ...
        var context=new FinanceCalculations.CancellationContext(at,at.plusSeconds(10),null,false,false);
    ...
    @Test void providerBookingCancellationIsNotFinalOrderFailure() {
        var at=Instant.parse("2026-09-14T00:00:00Z");
        var bookingOnly=new FinanceCalculations.CancellationContext(at,at,null,false,true);
    ...
    @Test void fullRefundIncludesPendingReservationsAndCannotOverRefund() {
        assertEquals("80.00",FinanceCalculations.remainingRefund("100","10","10"));
        assertThrows(IllegalStateException.class,()->FinanceCalculations.remainingRefund("100","50","51"));
    }
}
