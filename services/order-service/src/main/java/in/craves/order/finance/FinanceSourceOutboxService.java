...
public class FinanceSourceOutboxService {
    ...
    @Transactional public void complete(Work work,JsonNode response){
        if(!work.orderId().toString().equals(response.path("chefOrderId").asText()) || !response.path("result").isTextual())throw new IllegalStateException("Finance event acknowledgement does not match its order");
        String result=response.path("result").asText();if(result.length()>160)throw new IllegalStateException("Invalid finance acknowledgement");
        String status=result.startsWith("CONFLICT_")?"DEAD":"SENT";
        jdbc.update("UPDATE order_schema.finance_source_outbox SET status=?,last_result=?,lease_id=NULL,lease_until=NULL WHERE event_id=? AND lease_id=?",status,result,work.eventId(),work.leaseId());
    }
