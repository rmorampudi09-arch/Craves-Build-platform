...
- `main.bicep`: dormant template; must use a NEW approved app name and immutable image digest.
...
4. Export fresh full app inventory for the explicitly approved resource group. This is read-only; do not use inventory from another subscription or silently trust an old file.
...
- Apache Maven runtime classpath goal: https://maven.apache.org/plugins/maven-dependency-plugin/build-classpath-mojo.html

Compilation, disposable-database migration verification and static parameter tests are build evidence only. They are not Azure what-if, production deployment/migration, billing, networking or availability acceptance.
