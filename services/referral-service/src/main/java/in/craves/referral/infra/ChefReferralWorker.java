...
public class ChefReferralWorker {
    ...
    public void tick() {
        ...
        // Pausing new awards must not strand refund adjustments for already recorded earnings.
        for(var row:db.rows("SELECT s.order_id FROM referral_schema.order_snapshot s JOIN referral_schema.checkout c ON c.checkout_id=s.checkout_id JOIN referral_schema.policy p ON p.id=c.policy_id LEFT JOIN referral_schema.worker_schedule w ON w.kind='CHEF_EARN' AND w.aggregate_id=s.order_id WHERE p.program_kind='CHEF_COMMISSION_20260916' AND (? OR EXISTS(SELECT 1 FROM referral_schema.chef_reward r JOIN referral_schema.chef_posting x ON x.reward_id=r.id WHERE r.order_id=s.order_id)) AND (w.next_at IS NULL OR w.next_at<=?) ORDER BY w.next_at NULLS FIRST,s.order_id LIMIT 50",enabled,time(clock.instant()))) {
            var id=uuid(row,"order_id"); String code="WAITING_FOR_DEPENDENCY";
            try {if(earnings.process(id,enabled)) code="APPLIED";}
