...
public final class Json {
    ...
    public static String text(JsonNode value, String key, int maximum) {
        ...
        ReferralProblem.require(node != null && node.isTextual() && !node.textValue().isBlank()
            && node.textValue().length() <= maximum, 422, "INVALID_" + key.toUpperCase());
        return node.textValue();
    }
    public static String optionalText(JsonNode value, String key, int maximum) {
        JsonNode node = value.get(key);
        return node == null || node.isNull() ? null : text(value, key, maximum);
    }
    public static UUID uuid(JsonNode value, String key) {
        try { return UUID.fromString(text(value, key, 36)); }
        catch (IllegalArgumentException ex) { throw new ReferralProblem(422, "INVALID_UUID"); }
    }
    ...
    public static long money(JsonNode value, String key) {
        ...
        ReferralProblem.require(amount <= RewardPolicy.MAX_MONEY, 422, "MONEY_OUT_OF_RANGE");
        return amount;
    }
    public static int integer(JsonNode value, String key, int min, int max) {
        ...
        ReferralProblem.require(number >= min && number <= max, 422, "INTEGER_OUT_OF_RANGE");
        return number;
    }
    public static boolean bool(JsonNode value, String key) {
        ReferralProblem.require(value.has(key) && value.get(key).isBoolean(), 422, "INVALID_BOOLEAN");
        return value.get(key).booleanValue();
    }
    public static void fields(JsonNode value, String... fields) {
        ReferralProblem.require(value != null && value.isObject(), 422, "OBJECT_REQUIRED");
