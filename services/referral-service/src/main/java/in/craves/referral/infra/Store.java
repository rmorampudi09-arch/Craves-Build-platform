...
public class Store {
    ...
    public Map<String,Object> one(String sql,Object... parameters) {
        ...
        ReferralProblem.require(rows.size()==1,409,"SOURCE_DEPENDENCY_MISSING"); return rows.getFirst();
    }
    ...
    public void journal(String key,UUID user,long pending,long available,long reserved,String counterparty,UUID reward,UUID reference) {
        ...
        if(changed==0) {
            ...
            ReferralProblem.require(uuid(old,"user_id").equals(user) && number(old,"pending_delta")==pending
                ...
                && String.valueOf(old.get("counterparty")).equals(counterparty)
                && Objects.equals(uuid(old,"reward_id"),reward) && Objects.equals(uuid(old,"reference_id"),reference),409,"JOURNAL_KEY_CONFLICT");
        }
    }
    ...
    public void budget(String key,String track,long amount,String evidence) {
        ...
        List<Map<String,Object>> existing=rows("SELECT * FROM referral_schema.budget_journal WHERE event_key=?",key);
        if(!existing.isEmpty()) {
            Map<String,Object> old=existing.getFirst();
            ReferralProblem.require(number(old,"amount_paise")==amount && old.get("track").equals(track)
                && old.get("evidence_ref").equals(evidence),409,"BUDGET_KEY_CONFLICT"); return;
        }
        ...
        update("INSERT INTO referral_schema.budget_journal(event_key,track,amount_paise,evidence_ref) VALUES (?,?,?,?)",key,track,amount,evidence);
    }
    public void emit(String key,String type,Object payload) {
        String json=Json.write(payload);
        int added=update("INSERT INTO referral_schema.outbox(id,event_key,event_type,payload) VALUES (?,?,?,?::jsonb) ON CONFLICT(event_key) DO NOTHING",UUID.randomUUID(),key,type,json);
        if(added==0) {
            Map<String,Object> existing=one("SELECT event_type,payload FROM referral_schema.outbox WHERE event_key=?",key);
            ReferralProblem.require(type.equals(existing.get("event_type")) && Json.hash(Json.parse(json)).equals(Json.hash(Json.parse(existing.get("payload").toString()))),409,"OUTBOX_EVENT_KEY_CONFLICT");
        }
    }
